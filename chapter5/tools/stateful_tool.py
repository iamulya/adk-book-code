...
if __name__ == "__main__":
    runner = InMemoryRunner(agent=stateful_agent, app_name="StatefulApp"); session_id = "stateful_session_1"
    for i in range(3):
        user_message = Content(parts=[Part(text="Increment counter.")]); print(f"\nYOU: Increment counter. (Turn {i+1})")
        ...
        print()
        current_session = runner.session_service._get_session_impl(app_name="StatefulApp", user_id="state_user", session_id=session_id)
